...
seen = Set{}()
bodies = [b1, b2, b3, b4]
for i in range(1, stop=1000000)
	push!(seen, hash([(b.pos[1], b.vel[1]) for b in bodies]))
	global bodies
	bodies = [interact(b, bodies) for b in bodies]
	if in(hash([(b.pos[1], b.vel[1]) for b in bodies]), seen)
		println(i)
		break
	end
	println([(b.pos[1], b.vel[1]) for b in bodies])
end
